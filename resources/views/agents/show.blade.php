<x-layouts.app :heading="$agent->name">
    ...
    <section class="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">@foreach([['Successful',$learningSummary['successful_action'] ?? 0],['Unsuccessful',$learningSummary['unsuccessful_action'] ?? 0],['Inconclusive',$learningSummary['inconclusive_action'] ?? 0],['Proven patterns',$learningSummary['performance_pattern'] ?? 0]] as [$label,$value])<article class="rounded-lg bg-white p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]"><p class="text-xs text-slate-500">{{ $label }}</p><p class="mt-1 text-xl font-bold text-navy">{{ number_format($value) }}</p></article>@endforeach</section><a href="{{ route('agent-learning.index',['agent_id'=>$agent->id]) }}" class="mt-3 inline-flex text-sm font-semibold text-teal">View this agent's learning</a>
