...
<article class="rounded-lg bg-slate-50 p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0 max-w-4xl">
            ...
            <h2 class="mt-3 text-balance text-lg font-semibold text-navy">{{ $display($insight->title) }}</h2>
    ...
    @if($insight->affected_source_value)
        <p class="mt-3 break-all text-sm text-slate-500">Affected {{ str_replace('_', ' ', $display($insight->affected_source_type) ?: 'source') }}: <span class="font-medium text-slate-700">{{ $display($insight->affected_source_value) }}</span></p>
    @endif

    <div class="mt-4 grid gap-3 lg:grid-cols-3">
        <div class="rounded-lg bg-white p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">What is happening</p><p class="mt-2 text-sm leading-6 text-slate-700">{{ $display($insight->summary) }}</p></div>
        <div class="rounded-lg bg-white p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Why it matters</p><p class="mt-2 text-sm leading-6 text-slate-700">{{ $display($insight->why_it_matters) ?: 'This is tied to real search visibility and conversion potential.' }}</p></div>
        <div class="rounded-lg bg-white p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Best conversion action</p><p class="mt-2 text-sm font-medium leading-6 text-navy">{{ $display($insight->recommendation) }}</p></div>
    </div>

    <div class="mt-3 grid gap-3 lg:grid-cols-2">
        <div class="rounded-lg bg-white p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Expected result</p><p class="mt-2 text-sm leading-6 text-slate-700">{{ $display($insight->expected_result) ?: 'Clearer search visibility and a more focused conversion path.' }}</p></div>
        <div class="rounded-lg bg-white p-4 shadow-[0_0_0_1px_rgba(5,18,55,0.06)]"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Suggested task</p><p class="mt-2 text-sm leading-6 text-slate-700">{{ $display($insight->suggested_task) ?: 'Create a focused task from this insight.' }}</p></div>
    </div>
            ...
            <dl class="grid gap-3 border-t border-slate-100 p-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
                @foreach($insight->data_used as $label => $value)
                    @if(! is_null($value) && $value !== '')
                        <div><dt class="text-xs font-semibold uppercase text-slate-500">{{ str_replace('_', ' ', $display($label)) }}</dt><dd class="mt-1 break-all text-slate-700">{{ $display($value) }}</dd></div>
                    @endif
                @endforeach
            </dl>
        </details>
